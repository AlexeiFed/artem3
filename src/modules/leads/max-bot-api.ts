import "server-only";

import { request as httpsRequest } from "node:https";
import tls from "node:tls";

import { getServerEnv } from "@/lib/env/server";

/**
 * platform-api2.max.ru сидит на НУЦ Минцифры — Mozilla/macOS/Node его не знают.
 * Вшиваем публичные Root+Sub CA и allowPartialTrustChain (Node 22).
 */
const MAX_HOST = "platform-api2.max.ru";
const REQUEST_TIMEOUT_MS = 8_000;

const MINCIFRY_CA_BUNDLE = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`;

export type MaxSendResult =
  | { ok: true }
  | {
      ok: false;
      status: number;
      code?: string;
      description?: string;
    };

type HttpResult = { status: number; body: string };

function httpsJson(
  hostname: string,
  urlPath: string,
  body: string,
  extraHeaders: Record<string, string>,
): Promise<HttpResult> {
  return new Promise((resolve, reject) => {
    const headers: Record<string, string | number> = {
      Host: hostname,
      Accept: "application/json",
      "User-Agent": "artem-vibespace/1.0",
      "Content-Type": "application/json",
      "Content-Length": Buffer.byteLength(body),
      ...extraHeaders,
    };

    const req = httpsRequest(
      {
        hostname,
        port: 443,
        path: urlPath,
        method: "POST",
        headers,
        timeout: REQUEST_TIMEOUT_MS,
        ca: [...tls.rootCertificates, MINCIFRY_CA_BUNDLE],
        allowPartialTrustChain: true,
      },
      (res) => {
        const chunks: Buffer[] = [];
        res.on("data", (chunk: Buffer) => chunks.push(chunk));
        res.on("end", () => {
          resolve({
            status: res.statusCode ?? 0,
            body: Buffer.concat(chunks).toString("utf8"),
          });
        });
      },
    );

    req.on("timeout", () =>
      req.destroy(new Error(`MAX API timeout via ${hostname}`)),
    );
    req.on("error", reject);
    req.write(body);
    req.end();
  });
}

function parseMaxResponse(status: number, raw: string): MaxSendResult {
  let parsed: unknown;
  try {
    parsed = JSON.parse(raw) as unknown;
  } catch {
    return { ok: false, status, description: "invalid_json" };
  }

  if (
    status >= 200 &&
    status < 300 &&
    typeof parsed === "object" &&
    parsed !== null &&
    "message" in parsed &&
    typeof parsed.message === "object" &&
    parsed.message !== null
  ) {
    return { ok: true };
  }

  const record =
    typeof parsed === "object" && parsed !== null
      ? (parsed as Record<string, unknown>)
      : {};
  const code = typeof record.code === "string" ? record.code : undefined;
  const description =
    typeof record.message === "string" ? record.message : undefined;

  return {
    ok: false,
    status,
    ...(code === undefined ? {} : { code }),
    ...(description === undefined ? {} : { description }),
  };
}

export async function maxBotSendMessage(
  token: string,
  chatId: string,
  text: string,
): Promise<MaxSendResult> {
  const path = `/messages?chat_id=${encodeURIComponent(chatId)}&disable_link_preview=true`;
  const body = JSON.stringify({ text, format: "html" });
  const base = getServerEnv().MAX_API_BASE;
  const hostname = base ? new URL(base).hostname : MAX_HOST;
  const urlPath = base
    ? `${new URL(base).pathname.replace(/\/$/, "")}${path}`
    : path;

  const res = await httpsJson(hostname, urlPath, body, {
    Authorization: token,
  });
  return parseMaxResponse(res.status, res.body);
}
