...
interface HandlerDependencies {
  ...
  siteUrl: string;
  service: Pick<AdminContentService, "updateFaq" | "deleteFaq">;
}

export function createUpdateFaqHandler({
  requireAdmin,
  siteUrl,
  service,
}: HandlerDependencies): (
  ...
  return async function handleUpdateFaq(request, context) {
    ...
    try {
      const data = await service.updateFaq(id, body.value);
...
export function createDeleteFaqHandler({
  requireAdmin,
  siteUrl,
  service,
}: HandlerDependencies): (
  ...
  return async function handleDeleteFaq(request, context) {
    const blocked = await guardAdminMutation(request, {
      requireAdmin,
      siteUrl,
    });
