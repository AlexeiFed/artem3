...
interface HandlerDependencies {
  ...
  siteUrl: string;
  service: Pick<AdminContentService, "createFaq">;
}

export function createCreateFaqHandler({
  requireAdmin,
  siteUrl,
  service,
}: HandlerDependencies): (request: Request) => Promise<Response> {
  return async function handleCreateFaq(request: Request): Promise<Response> {
    ...
    try {
      const data = await service.createFaq(body.value);
