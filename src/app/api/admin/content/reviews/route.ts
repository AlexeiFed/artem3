...
interface HandlerDependencies {
  ...
  siteUrl: string;
  service: Pick<AdminContentService, "createReview">;
}

export function createCreateReviewHandler({
  requireAdmin,
  siteUrl,
  service,
}: HandlerDependencies): (request: Request) => Promise<Response> {
  return async function handleCreateReview(
    request: Request,
  ): Promise<Response> {
    ...
    try {
      const data = await service.createReview(body.value);
...
export async function POST(request: Request): Promise<Response> {
  ...
  return createCreateReviewHandler({
    requireAdmin,
